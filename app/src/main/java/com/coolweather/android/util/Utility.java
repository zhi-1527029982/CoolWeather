        ...
        if (!TextUtils.isEmpty(response)) {  // 数据非空
            try {
                ...
                for (int i = 0; i < allProvinces.length(); i++) {
                    ...
                    province.setProvinceName(provinceObject.getString("name"));  // 将 JSON 对象中的 name 字段赋值给 ProvinceName
                    province.setProvinceCode(provinceObject.getInt("id"));  // 将 JSON 对象中的 id 字段赋值给 ProvinceCode
